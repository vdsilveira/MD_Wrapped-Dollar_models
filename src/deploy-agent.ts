...
import { resolveNetwork, getOrCreateSeed, recordDeployment, saveWdollarAgentAddress, saveWdollarAgentAccountId } from './network';
...
const accountId = persistentHash(new CompactTypeVector(1, Bytes32Descriptor), [secretKey]);
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy Wrapped Dollar for Agents to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      deployed = await deployContract(providers, {
        ...
        args: [
          'Wrapped Dollar for Agents',
          'WDA',
          6n,
          eitherAddress(accountId),
        ],
  ...
  saveWdollarAgentAddress(contractAddress);
  saveWdollarAgentAccountId(Buffer.from(accountId).toString('hex'));
  console.log('  Saved to .midnight-state.json\n');

  await persistWalletState(network, walletCtx);
  await walletCtx.wallet.stop();
  console.log('─── Deployment complete ────────────────────────────────────────\n');
  console.log('  Next: npm run cli:agent\n');
}
